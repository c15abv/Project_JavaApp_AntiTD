...
public class TowerFigureTemplate {
	...
	public TowerFigure createNewTower(Position position) {
		return towerType == Figures.CIRCLE ? createNewCircleTower(position)
				: (towerType == Figures.SQUARE ? createNewSquareTower(position)
						: (towerType == Figures.TRIANGLE
								? createNewTriangleTower(position)
								: createNewStarTower(position)));
	}

	private CircleTowerFigure createNewCircleTower(Position position) {
		return new CircleTowerFigure(baseDamage, hue, range, cooldown,
				position);
	}

	private SquareTowerFigure createNewSquareTower(Position position) {
		return new SquareTowerFigure(baseDamage, hue, range, cooldown,
				position);
	}

	private TriangleTowerFigure createNewTriangleTower(Position position) {
		return new TriangleTowerFigure(baseDamage, hue, range, cooldown,
				position);
	}

	private StarTowerFigure createNewStarTower(Position position) {
		return new StarTowerFigure(baseDamage, hue, range, cooldown, position);
	}
}
