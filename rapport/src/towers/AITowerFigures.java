...
public class AITowerFigures {
	...
	public AITowerFigures(AttackingPlayer attacker, DefendingPlayer defender) {
	...
	public void update() {
		...
		for (TowerFigure tower : currentDefence) {
			if (!tower.isOnCooldown() && (!tower.hasTarget()
					|| (tower.hasTarget() && (!tower.getTarget().isAlive()
							|| tower.getTarget().hasReachedGoal()
							|| !targetWithinRange(tower,
									tower.getTarget()))))) {
				findTarget(tower);
			}
	...
	public static double getTargetDistance(Position towerPosition,
		...
		return Math.sqrt(Math.pow(towerPosition.getY() - targetPosition.getY(),
				2) + Math.pow(towerPosition.getX() - targetPosition.getX(), 2));
	}

	private boolean betterCompared(TowerFigure tower, CreatureFigure oldTarget,
