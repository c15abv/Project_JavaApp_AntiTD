...
public class TeleportTile extends PathTile {
	...
	/**
	 * Constructor that initiates the tile and sets the position and path to the
	 * given parameters.
	 *
	 * @param position
	 *            The tile's position
	 * @param path
	 *            The tile's ValidPath
	 */
	public TeleportTile(Position position, ValidPath path) {
		super(position, path);
		renderAnimationCount = renderAnimationCount2 = sleepCount = sleepCount2 = 0;
	...
	public void landOn(CreatureFigure creature) {
		if (connection != null
				&& creature.getPosition().equals(getTeleportPosition())) {
	...
	public void render(Graphics2D g2d) {
		...
		alpha2 = 1.0f
				- (float) renderAnimationCount2 / (float) RENDER_COUNT_LIMIT2;
		size = (3 * Tile.size) / 4 - (int) ((3 * Tile.size) / 4
				* (double) renderAnimationCount / RENDER_COUNT_LIMIT);
		size2 = Tile.size / 4 + (int) ((Tile.size / 4)
				* (double) renderAnimationCount2 / RENDER_COUNT_LIMIT2);
		if (connection != null)
			color = new Color((float) 85 / 255, (float) 175 / 255,
					(float) 230 / 255, alpha);
	...
	public void setTeleporterAt(Position position) {
		if (position != null && AreaPosition.withinArea(getPosition(), position,
				Tile.size, Tile.size)) {
			teleportPosition = position;
		}
	}
