...
public class Game extends Canvas
		implements TimerListener, MouseListener, MouseMotionListener {
	...
	private int mouseCoordinateX, mouseCoordinateY, cameraOffsetX,
			cameraOffsetY, canvasWidth, canvasHeight;
	...
	/**
	 * Creates a new game with the specified level. Default width and height are
	 * used.
	 *
	 * @param level
	 *            The level to initiate the game with.
	 */
	...
	/**
	 * Creates a new game with the specified level, with the specified width and
	 * height.
	 *
	 * @param level
	 *            The GameLevel.
	...
	public Game(GameLevel level, int canvasWidth, int canvasHeight) {
		...
		mouseCoordinateX = mouseCoordinateY = cameraOffsetX = cameraOffsetY = 0;
	...
	/**
	 * Updates the logic of any object in the game, as well as updating the game
	 * camera/view.
	 */
	public void update() {
		if (gameState != GameState.ENDED) {
			try {
				...
				if (gameState == GameState.RUNNING) {
					defender.update();
					attacker.update();
					if (attacker.getPoints() >= level
							.getAttackingPlayerScoreGoal()) {
	...
	/**
	 * Renders any objects in the game with a double buffering strategy.
	 */
	public void render() {
		if (gameState != GameState.ENDED) {
			...
			g2d.fillRect(0, 0, level.getWidth() + Tile.size + cameraOffsetX,
	...
	/*
	 * The width of the level which is hidden due to the canvas being shorter in
	 * width than the level width.
	 */
	...
	/**
	 * The height of the level which is hidden due to the canvas being shorter
	 * in height than the level height.
	 *
	 * @return
	 */
	...
	private void updateCanvasCamera() {
		if (mouseCoordinateY < canvasHeight / 2 + canvasHeight / 6
			...
			if (mouseCoordinateX > canvasWidth - 40
				...
				cameraOffsetX += 4;
			} else if (mouseCoordinateX < 40 && cameraOffsetX > 0) {
				cameraOffsetX -= 4;
			}
			...
			if (mouseCoordinateY > canvasHeight - 40
				...
				cameraOffsetY += 4;
			} else if (mouseCoordinateY < 40 && cameraOffsetY > 0) {
				cameraOffsetY -= 4;
			}
		}
	...
	public void startGame() {
		timerThread.start();
		timer.setTimer(gameTimeTimerId, this, level.getTimeToFinish() * 1000);
	...
	public void mouseClicked(MouseEvent arg0) {
		Position position = level.getAdjacentStartPosition(
				arg0.getX() + cameraOffsetX + Tile.size / 2,
